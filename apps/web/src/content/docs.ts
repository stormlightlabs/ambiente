...
export type DocumentFrontmatter = { description?: string; order?: number; title?: string };
...
type MarkdownModule = { default: string; frontmatter?: DocumentFrontmatter };
...
const entries = Object.entries(modules).map(([sourcePath, module]) => {
	...
	if (typeof module.default === 'string') {
		return { ...common, component: undefined, headings: headingsFromHtml(module.default), html: module.default };
	}

	const mdxModule = module as MdxModule;
	return { ...common, component: mdxModule.default, headings: mdxModule.headings ?? [], html: undefined };
});
...
export function findDocumentation(slug: string): DocumentationEntry | undefined {
