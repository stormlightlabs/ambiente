			...
			{(current) => (
				<div class="docs-layout">
					<aside class="docs-sidebar" aria-label="Documentation">
						...
						<nav>
							<For each={documentation}>
								{(entry) => (
									<a href={entry.path} aria-current={entry.slug === current().slug ? 'page' : undefined}>
										{entry.title}
									</a>
								)}
							</For>
					...
					<article class="prose" data-pagefind-body>
						<Show when={current().component} fallback={<div innerHTML={current().html ?? ''} />}>
							{(component) => <Dynamic component={component()} components={mdxComponents} />}
						</Show>
					</article>

					<Show when={pageHeadings().length > 0}>
						<aside class="page-nav" data-pagefind-ignore>
							<p>On this page</p>
